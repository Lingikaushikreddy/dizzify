...
import app.launcher.data.AnimationConstants
import app.launcher.data.AppModel
import app.launcher.data.Constants
import app.launcher.data.AnimationConfig
import app.launcher.data.AppKey
...
suspend fun getAppsList(
    ...
    try {
        ...
        when (settings.sortOrder) {
            app.launcher.settings.SortOrder.Recent -> {
                appList.sortWith(
                    compareByDescending<AppModel> { it.lastLaunchTime }
                        .thenBy { it.appLabel.lowercase() }
                )
            }
            app.launcher.settings.SortOrder.ZA -> {
    ...
    try {
        ...
        try {
            val intent = Intent(Intent.ACTION_MAIN).setClassName(
                context,
                "app.launcher.helper.FakeHomeActivity"
            )
